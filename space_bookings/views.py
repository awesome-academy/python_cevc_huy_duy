...
class SpaceBookingDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        booking = SpaceBooking.objects.select_related(
            'user', 'space', 'space__working_space'
        ).get(**filters)
    ...
    def update(self, request, *args, **kwargs):
        ...
        updated_instance = instance.update_booking(**serializer.validated_data)
        
        return Response({
            'message': BookingMessages.UPDATE_SUCCESS,
            'booking': SpaceBookingSerializer(updated_instance).data
        }, status=status.HTTP_200_OK)
...
class SpaceBookingCancelView(generics.UpdateAPIView):
    ...
    def get_object(self):
        ...
        booking = SpaceBooking.objects.select_related('user', 'space').get(**filters)
    ...
    def update(self, request, *args, **kwargs):
        ...
        canceled_instance = instance.cancel_booking()

        return Response({
            'message': BookingMessages.CANCEL_SUCCESS,
            'booking': SpaceBookingSerializer(canceled_instance).data
        }, status=status.HTTP_200_OK)
